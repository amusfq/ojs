...
class NativeXmlSubmissionFileFilter extends NativeImportFilter
{
    ...
    public function handleRevisionElement($node)
    {
                ...
                switch ($childNode->tagName) {
                    case 'href':
                        if (php_sapi_name() !== 'cli') {
                            $deployment->addError(PKPApplication::ASSOC_TYPE_SUBMISSION, $submission->getId(), 'Web-based imports may not specify "href"; use "embed" instead, or import using the command line.');
                            break;
                        }
        ...
        if ($temporaryFilename) {
            ...
            if (!preg_match('#^[a-zA-Z]+(\.[a-zA-Z]+)*$#', $extension)) {
                $extension = 'txt';
            } // pkp/pkp-lib#13030
