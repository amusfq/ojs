...
<main class="thalibul-auth min-h-[calc(100vh-8rem)] bg-[#fff8fb] px-4 py-10 lg:px-12 lg:py-16">
	<div class="mx-auto grid max-w-5xl overflow-hidden border border-[#f4dce7] bg-white shadow-[0_20px_60px_rgba(157,57,83,.10)] lg:grid-cols-[.85fr_1.15fr]">
		<section class="relative hidden overflow-hidden bg-[#9d3953] p-10 text-white lg:flex lg:flex-col lg:justify-between">
			...
			<div class="relative"><a href="{url page="index" router=$smarty.const.ROUTE_PAGE}" class="text-sm font-bold uppercase tracking-[.2em] text-white no-underline">{$currentContext->getLocalizedName()|escape}</a><p class="mt-20 max-w-xs text-4xl font-semibold leading-tight">This link needs a fresh start.</p></div>
			<p class="relative max-w-xs text-sm leading-6 text-white/75">Password-reset links are temporary for your account’s security.</p>
		</section>
		<section class="p-6 sm:p-10">
			...
			<div class="grid h-14 w-14 place-items-center rounded-full bg-red-50 text-2xl font-semibold text-red-600" aria-hidden="true">!</div>
			...
			<div class="mt-4 text-sm leading-7 text-slate-600">{translate key=$errorMsg params=$errorParams}</div>
